...
const HackdayProgress = () => {
    ...
    const status5 = date > dataList.step7? "finish" : date > dataList.step6? "process" : "wait";

    return (
        <Card bordered={false} title="比赛进程">
            <Steps progressDot={true} direction="vertical">
                <Steps.Step status={status1} title="6月16日 - 6月30日" description="开始报名 & 筛选简历" />
                <Steps.Step status={status2} title="6月30日" description="结束报名 & 确认参赛队伍" />
                <Steps.Step status={status3} title="7月1日 - 7月5日" description="线上开发" />
                <Steps.Step status={status4} title="7月6日" description="确认线下 Hackday 队伍" />
                <Steps.Step status={status5} title="7月9日" description="线下 Hackday" />
            </Steps>
        </Card>
    );
};
